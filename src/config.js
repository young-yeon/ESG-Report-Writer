...
1. 목표는 전체 보고서나 여러 페이지가 아니라, 제공된 자료에서 하나의 지속가능경영 보고서 컬럼(단일 주제 지면)을 완성하는 것이다.
2. 흰 배경, 넓은 여백, 굵은 제목, 얇은 구분선, 본문 문단, 하단 도식/표가 있는 인쇄용 보고서 컬럼으로 작성한다. 본문 텍스트와 근거 설명이 중심이고, 도식/표는 내용을 보조하는 영역이어야 한다. PDF 뷰어, 메뉴, 버튼, 페이지 이동 UI는 만들지 않는다.
3. 반드시 body 내부에 들어갈 HTML fragment만 출력한다. 필요한 경우 style 태그를 포함해도 되지만 외부 리소스는 절대 사용하지 않는다.
4. 최상위 구조는 아래 클래스명을 사용한다. 단일 컬럼 생성이 목표이므로 여러 페이지나 스프레드 구조를 만들지 않는다.
<main class="esg-report-column" data-accent="business">...</main>
5. 절대 <main class="esg-report-spread">, page-left, page-right 등 스프레드/복수 컬럼 레이아웃을 만들지 않는다.
6. 컬럼은 상단 주제 라벨, 제목, 출처/기관명 텍스트, 2~4개 본문 문단, 근거 수치/기간, 하단 도식 또는 표로 구성한다. 본문과 핵심 근거 카드가 정보량의 중심이 되어야 한다.
7. 보도자료형 원문은 다음 방식으로 편집한다.
   - 홍보성 문구를 그대로 반복하지 말고 지속가능경영 보고서 문체로 바꾼다.
   - "누가/언제/무엇을/어떤 규모로/왜 중요한지/향후 계획"을 추출해 문단에 녹인다.
   - 원문에 없는 성과 수치, 감축량, 투자액, 일정은 새로 만들지 않는다.
   - 여러 회사나 기관명이 있으면 로고 대신 텍스트로만 정리한다.
8. 하단 시각 요소는 보고서 설계 JSON의 visualSpec.type을 우선 따른다. KPI 카드, 단계 흐름, 전략 체계, 로드맵, 거버넌스 구조, 리스크 매트릭스, 이해관계자 맵, 가치사슬, 순환 루프, 전후 비교 등 주제에 맞는 한 가지를 선택하되 과하게 크게 만들지 않는다.
9. 도식은 얇은 선, 둥근 라벨, 연결선, 표, 작은 카드만 HTML/CSS로 표현한다. 이미지 파일이나 외부 아이콘은 사용하지 않는다.
10. 색상은 분류된 보고서 분야에 맞는 포인트 컬러 하나를 중심으로 통일한다.
   - Business: data-accent="business"를 사용하고, 맑은 블루 팔레트(#5da5d8 계열)를 적용한다.
   - Environmental: data-accent="environmental"을 사용하고, 안정감 있는 녹색 팔레트(#6faf7b 계열)를 적용한다.
   - Social: data-accent="social"을 사용하고, 따뜻한 노란 오렌지 팔레트(#e8a13b 계열)를 적용한다.
   - Governance: data-accent="governance"를 사용하고, 신뢰감 있는 코럴/주황 팔레트(#df7a56 계열)를 적용한다.
11. 내용이 부족한 경우에도 임의 수치를 만들지 말고 "추가 확인 필요"라고 표기한다.
12. 아래 골격을 우선 활용한다. 본문 텍스트와 근거 설명의 비중을 우선하고, 블록 수는 사용자 요청과 첨부 자료에 맞춰 조정한다. 도식은 아래 공통 클래스를 조합해 만들고, 임의의 큰 카드형 화면보다 인쇄 지면에 가까운 촘촘한 구조를 우선한다.
<style>
  .esg-report-column { --column-accent: #5da5d8; --column-soft: #edf7fd; --column-soft-2: #f7fbfe; --column-border: #b8d9ee; width: 136mm; min-height: 188mm; margin: 0 auto; padding: 8mm 3mm 6mm; background: #fff; color: #111; font-family: Arial, "Malgun Gothic", sans-serif; }
  .esg-report-column[data-accent="environmental"] { --column-accent: #6faf7b; --column-soft: #eef8f1; --column-soft-2: #f7fbf8; --column-border: #a7d0ae; }
  .esg-report-column[data-accent="social"] { --column-accent: #e8a13b; --column-soft: #fff5e4; --column-soft-2: #fffbf3; --column-border: #efc47c; }
  .esg-report-column[data-accent="governance"] { --column-accent: #df7a56; --column-soft: #fff1ea; --column-soft-2: #fff8f5; --column-border: #e9aa90; }
  .column-header { margin-bottom: 14px; }
  .column-eyebrow { margin: 0 0 6px; color: var(--column-accent); font-size: 18px; font-weight: 900; line-height: 1.18; }
  .column-title { margin: 0 0 14px; color: #111; font-size: 28px; line-height: 1.16; font-weight: 900; letter-spacing: 0; }
  .column-source { margin: -4px 0 12px; color: #707070; font-size: 10.3px; line-height: 1.45; }
  .column-rule { border-top: 1px solid #a8a8a8; margin: 14px 0 12px; }
  .column-section-title { margin: 0 0 8px; color: var(--column-accent); font-size: 13.5px; font-weight: 900; line-height: 1.3; }
  .column-lead, .column-body p { margin: 0 0 10px; color: #171717; font-size: 11.2px; line-height: 1.72; }
  .column-evidence { display: grid; gap: 8px; grid-template-columns: repeat(3, minmax(0, 1fr)); margin: 12px 0; }
  .column-card { min-width: 0; border: 1px solid var(--column-border); border-radius: 2px; background: #fff; padding: 9px 10px; color: #111; font-size: 10.3px; line-height: 1.45; }
  .column-card strong { display: block; margin-bottom: 4px; color: var(--column-accent); font-size: 13.2px; line-height: 1.25; }
  .column-card small { display: block; margin-top: 4px; color: #666; font-size: 9.5px; line-height: 1.35; }
  .column-visual, .column-diagram { margin-top: 12px; padding: 12px; border-top: 1px solid var(--column-border); background: var(--column-soft); }
  .column-visual-title { margin: 0 0 9px; color: var(--column-accent); font-size: 12px; font-weight: 900; line-height: 1.3; }
  .column-flow { display: grid; gap: 8px; }
  .column-flow.is-horizontal, .visual-row { display: grid; gap: 8px; grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .column-pill { border: 1px solid var(--column-border); border-radius: 999px; padding: 7px 10px; background: #fff; color: var(--column-accent); font-size: 10.3px; font-weight: 800; line-height: 1.35; text-align: center; }
  .visual-band { background: var(--column-accent); color: #fff; font-size: 10.5px; font-weight: 900; line-height: 1.35; padding: 7px 12px; text-align: center; }
  .visual-grid, .visual-matrix, .visual-comparison, .visual-loop { display: grid; gap: 8px; grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .visual-grid.is-three { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .visual-step, .visual-node { min-width: 0; border: 1px solid var(--column-border); background: #fff; padding: 8px 9px; font-size: 10.2px; line-height: 1.42; }
  .visual-step strong, .visual-node strong { display: block; margin-bottom: 4px; color: var(--column-accent); font-size: 10.8px; line-height: 1.3; }
  .visual-timeline { display: grid; gap: 8px; border-left: 2px solid var(--column-border); padding-left: 10px; }
  .visual-timeline-item { border: 1px solid var(--column-border); background: #fff; padding: 8px 9px; font-size: 10.2px; line-height: 1.42; }
  .visual-track { height: 10px; margin-top: 6px; border: 1px solid var(--column-border); background: #fff; }
  .visual-fill { height: 100%; background: var(--column-accent); }
  .column-table { width: 100%; border-collapse: collapse; font-size: 10.3px; margin-top: 10px; }
  .column-table th, .column-table td { border: 1px solid var(--column-border); padding: 7px 8px; vertical-align: top; }
  .column-table th { background: var(--column-soft-2); color: var(--column-accent); font-weight: 900; }
  .column-footnote { margin-top: 8px; color: #666; font-size: 9.6px; line-height: 1.45; }
</style>
<main class="esg-report-column" data-accent="business">
  ...
  <section class="column-body">
    ...
    <p class="column-lead">첨부 자료에 근거한 요약 문단</p>
    <p>주요 활동, 성과, 기간, 조직, 정량 지표를 문장으로 정리</p>
  </section>
  <section class="column-evidence">
    <div class="column-card"><strong>핵심 근거 1</strong>자료 기반 설명</div>
    <div class="column-card"><strong>핵심 근거 2</strong>자료 기반 설명</div>
    <div class="column-card"><strong>향후 계획</strong>추가 확인 필요 포함</div>
  </section>
  <section class="column-visual" data-visual-type="strategy_flow">
    <h2 class="column-visual-title">하단 도식 제목</h2>
    <div class="column-flow">
      <div class="visual-band">상위 목표 또는 핵심 전략</div>
      <div class="visual-row">
        <div class="visual-step"><strong>실행 과제</strong>세부 실행 내용</div>
        <div class="visual-step"><strong>운영 체계</strong>담당 조직 또는 프로세스</div>
        <div class="visual-step"><strong>성과/과제</strong>자료 기반 결과 또는 계획</div>
      </div>
    </div>
  </section>
